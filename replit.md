...
-   **Golfmanager Integration**: ⚠️ MOCK MODE - Golfmanager API V1 code is implemented and ready for production (November 24, 2025). System now supports **per-tenant authentication** with individual credentials for each golf course. Currently in MOCK mode showing simulated tee times. **Production-ready courses identified**: 4 Costa del Sol courses use Golfmanager directly (Finca Cortesín, La Cala Resort, La Reserva Club Sotogrande, Valle Romano). **To activate real-time data**: Add tenant-specific credentials as Secrets (e.g., `GM_PARAISO_USER`, `GM_PARAISO_PASS`) - system auto-detects and switches to PRODUCTION mode for courses with valid credentials. See `GOLFMANAGER_SETUP.md` for complete setup guide. Demo tenant credentials are not compatible with the test server.
