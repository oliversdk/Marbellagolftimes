...
-   **Golfmanager Integration**: ⚠️ MOCK MODE - Golfmanager API V1 code is implemented and ready for production (November 24, 2025). Currently in MOCK mode showing simulated tee times. **Production-ready courses identified**: 4 Costa del Sol courses use Golfmanager directly (Finca Cortesín, La Cala Resort, La Reserva Club Sotogrande, Valle Romano). **To activate**: Contact Golfmanager to get production tenant credentials for these 4 courses, then update environment variables and switch to PRODUCTION mode. Demo tenant credentials are not compatible with the test server and cannot be used for preview.
