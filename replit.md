...
-   **TeeOne/Golfmanager Integration**: 17 premium courses use TeeOne booking system (Golfmanager backend) displayed as direct booking links (`DEEP_LINK` type) to TeeOne's frontend at `https://open.teeone.golf/en/{tenant}/disponibilidad`. Notable courses include Marbella Golf, Santa Clara, Finca Cortesín, Real Club Valderrama, Los Naranjos, La Quinta, Los Arqueros, Estepona Golf, Torrequebrada, La Reserva Sotogrande, Real Club de Golf Sotogrande, and La Cala Resort. When Golfmanager API credentials become available, this can be upgraded to show real-time availability directly in the platform.
