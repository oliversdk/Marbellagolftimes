...
-   **Golfmanager Integration**: ✅ ACTIVE - Golfmanager API V1 successfully integrated with demo credentials (November 24, 2025). Currently in DEMO mode showing demo tee times with graceful fallback: when a course's specific tenant is not accessible, the system automatically falls back to the demo tenant and displays demo times with a clear note "Demo times shown - Real availability coming soon". This provides users with a functional preview of the real-time booking system. **Production-ready courses identified**: 4 Costa del Sol courses use Golfmanager directly (Finca Cortesín, La Cala Resort, La Reserva Club Sotogrande, Valle Romano). Awaiting production tenant access from Golfmanager to enable real-time availability for these courses.
-   **TeeOne Golf System**: Separate booking platform (teeone.golf - Madrid-based) used by 9 Costa del Sol courses (Marbella Golf, Santa Clara, Los Naranjos, La Quinta, Los Arqueros, Atalaya, Mijas, Torrequebrada, Villa Padierna). This is a DISTINCT system from Golfmanager and requires separate API integration. These courses currently show direct booking links to `https://open.teeone.golf/en/{tenant}/disponibilidad`.
...
-   **Golfmanager Operating Modes**: System supports DEMO mode (using demo tenant with automatic fallback for inaccessible tenants), MOCK mode (simulated data for testing), and PRODUCTION mode (real-time API with production credentials).
